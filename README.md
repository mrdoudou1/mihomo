...
`🚀 手动选择` 中只列出订阅入口，例如 `📦 订阅 1`、`📦 订阅 2`，不再混入全部节点。先选择订阅，再展开对应订阅组选择节点。完整路径为 `GLOBAL → 🎯 选择节点 → 🚀 手动选择 → 📦 订阅 1 → 节点`。

`🎯 选择节点` 只保留直连、自动选择、手动选择三种入口。自动选择仍可跨订阅测速；所有订阅节点都会加上 `[订阅 1]` 等前缀，因此自动选择结果和连接记录也能辨认来源。订阅组仍会显示为独立卡片，这是面板对策略组的展示方式；手动选择内部已改为订阅入口。
